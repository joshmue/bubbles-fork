...
const FALLBACK_ICON: &[u8] = include_bytes!("../de.gonicus.Bubbles.svg");
...
pub async fn prepare_install(
    ...
    // The portal takes a GBytesIcon and nothing else: it deserialises the
    // variant, checks G_IS_BYTES_ICON, and runs the bytes through its icon
    // validator. A themed icon fails that check, so an application without an
    // icon of its own is exported under ours rather than a named fallback.
    let bytes = icon.unwrap_or_else(|| FALLBACK_ICON.to_vec());
    let serialized_icon = gio::BytesIcon::new(&glib::Bytes::from_owned(bytes))
        .serialize()
        .expect("a bytes icon to serialise");
    // The argument is a boxed `v`, not the `(sv)` a serialised GIcon already
    // is; the portal unboxes it before looking at the icon inside.
    let icon_variant = Variant::from_variant(&serialized_icon);
